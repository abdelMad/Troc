jQuery(function ($) {
    ...
    if ($changePwd.length) {
        ...
        $changePwd.on('click', function (e) {
            ...
            util.jsonAjax('/forgot-password', JSON.stringify({
                email: email,
                pwd: pwd,
                confPwd: confPwd
            }), function (data) {
                console.log(data);
                if(data.length){
                    if(data[0]==='mailNotFound'){
                        alert("email n'existe pas!");
                        $('#pwdModal').modal('hide');
                    }else if(data[0] === 'pwdNotMatch'){
    ...
    if($notifs.length){
        $notifs.on('click',function (e) {
            e.preventDefault();
            var self = $(this);
            util.jsonAjax('/set-notification-status',JSON.stringify([self.data('notif')]),function (data) {
                console.log(data);
