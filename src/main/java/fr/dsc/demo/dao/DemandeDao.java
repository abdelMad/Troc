...
public interface DemandeDao extends JpaRepository<Demande,Integer> {
    ...
    @Query("SELECT DISTINCT count(d) FROM Message m JOIN Demande d  ON m.demande=d JOIN Utilisateur u ON m.recepteur=u WHERE  u.email=(:email)")
    Long existsByEmail(@Param("email") String email);

    @Query("SELECT DISTINCT d.numAuth FROM Message m JOIN Demande d  ON m.demande=d JOIN Utilisateur r ON m.recepteur=r JOIN Utilisateur em ON m.emetteur = em WHERE (em.email=(:emetteur) AND r.email=(:recepteur)) OR (r.email=(:emetteur) AND em.email=(:recepteur))")
